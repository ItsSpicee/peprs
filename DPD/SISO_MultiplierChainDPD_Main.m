...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
SignalName = '5G_NR_OFDM_400MHz_256QAM_Pilots';
% Load in the signal
...
RX.Fcarrier             = 2.002e9;      % Center frequency of the received modulated signal
...
% Multiplier Chain DPD Files
DPD.MultiplierChainFlag = 0;
DPD.MultiplierAFile = '.\Multiplier_Chain_DPD_Files\5G_NR_400MHz_256QAM_DPDA_Memoryless_June2.mat';
...
% RX Calibration file
Cal.RX.CalFile = '.\RX_CalResults\RX_CalResults_fc25GHz_fLORX_27r002GHz_fIF_2r002GHz_BW3GHz.mat';
...
for IterationCount = 1:(DPD.NofIteration+1)
    ...
    if DPD.MultiplierChainFlag
        [ In_ori, memTruncA ] = ApplyDoublerDPD(In_ori, DPD.MultiplierAModel);
        In_ori = [In_ori((end-(memTruncA-1)):end,1); In_ori];
    end
...
TX.AWG.ExpansionMarginSettings.ExpansionMargin = 5.1;
UploadSignal
if Cal.Processing32BitFlag
...
switch DPD.Type
    case 'SquareRootBasis'
        ...
        [a, b] = AdjustPhase(In_D_EVM.^4,Out_D_EVM);
        [~, ~, PAPRsqout] = CheckPower(a,0);
        a = SetMeanPower(a,PAPRsqin-PAPRsqout);
        PlotGain(a, b*10^((PAPRsqin-PAPRsqout+5)/20));
        xlim([-20,11])
        ylim([-6,10])
        [a, b] = AdjustPhase(a, b);
        [a, b] = AdjustPhase(a, b);
