function [ VolterraCoeff , StaticCoeff ] = VolterraDpdIdentification_IdentifyVolterraCoeff_ET( data , VolterraParameters )
    Vin  = data.In  ;
    Vout = data.Out ;
    Vdd = data.Vdd ;
...
%     figure()
%     plot(abs(Vin)); hold on; plot(Vdd, 'r');
...
%% 11th Kernal
    ...
    DebugPrint(['Conditioning number ' num2str(cond(Ut))]);
    size(Ut);
%     size(Vdd)
    ...
    Cond_A = cond(U);
%      figure()
%     plot(abs(Vin)/max(abs(Vin))); hold on;
%     plot(Vdd/max(Vdd),'r')
%     plot(abs(U(:,65)),'r')  
    
%% Compute the Volterra Coefficients
    DebugPrint( 'Identify Volterra coefficients' ) ;
    U             = double( U )         ;    
    [ Q , R ]     = qr( U , 0 )         ;
...
%% Compute the Static Model Coefficients        
    ...
    DebugPrint( 'Identify static coeffifients' ) ;
