    ...
    if (fs ~= 0)
        if (isfield(arbConfig, 'clockSource'))
            switch (arbConfig.clockSource)
                ...
                case 'IntRef'
                    xfprintf(f, sprintf(':ROSC:SOURce INT;:OUTP:ROSC:SOURce SCLK2;:FREQuency:RASTer %.15g;', fs));
                case 'AxieRef'
                    xfprintf(f, sprintf(':ROSC:SOURce AXI;:OUTP:ROSC:SOURce SCLK2;:FREQuency:RASTer %.15g;', fs));
                case 'ExtRef'
                    ...
                    if (range == 3) % do not set :FREQ:RAST command in range 3
                        xfprintf(f, sprintf(':ROSC:SOURce EXT;RANGe RANG%d;FREQuency %.15g;:OUTP:ROSC:SOUR EXT', range, arbConfig.clockFreq));
                        ...
                        xfprintf(f, sprintf(':ROSC:SOURce EXT;RANGe RANG%d;FREQuency %.15g;:OUTP:ROSC:SOURce SCLK2;:FREQuency:RASTer %.15g;', range, arbConfig.clockFreq, fs));
                    end
                case 'ExtClk'
