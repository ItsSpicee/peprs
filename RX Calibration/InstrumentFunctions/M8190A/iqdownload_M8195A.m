function result = iqdownload_M8195A(arbConfig, fs, data, marker1, marker2, segmNum, keepOpen, channelMapping, sequence, run, segmentLength, segmentOffset)
...
if (~isempty(sequence))
    ...
    if (run ~= 0 && sum(sum(channelMapping)) ~= 0 && segmentOffset + length(data) >= segmentLength)
        if (run > 0)
