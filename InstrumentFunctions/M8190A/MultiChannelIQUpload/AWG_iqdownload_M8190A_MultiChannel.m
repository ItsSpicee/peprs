function result = AWG_iqdownload_M8190A_MultiChannel(arbConfig, fs, data, marker1, marker2, segmNum, keepOpen, channelMapping, sequence, run)
...
% THIS SOFTWARE MAY ONLY BE USED IN CONJUNCTION WITH KEYSIGHT INSTRUMENTS. 
    ...
    if (size(channelMapping, 1) > 2 && isfield(arbConfig, 'visaAddr2')) % && max(max(channelMapping(3:4,:))) > 0)
        ...
        result = AWG_iqdownload_M8190A_MultiChannel(arb2, fs, data, marker1, marker2, segmNum, keepOpen, channelMapping(3:4,:), sequence, run);
...
%                if (xfprintf(f, cmd, useM8192A) ~= 0)
        ...
        %% Modified to be done outside of this function
