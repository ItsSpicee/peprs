...
% Last Modified by GUIDE v2.5 20-Jun-2017 10:15:43
...
modelInfo = { ...
% tag              mode (empty if no modes)           ptr to arbModel
'M8196A',          [],                                      0; ...
...
%try
    if (exist('arbConfig', 'var'))
        if (isfield(arbConfig, 'model'))
            ...
            idx = find(strcmp(modelInfo(:,1), arbConfig.model));
            if (idx > 0)
        ...
        if (isfield(arbConfig, 'pxieSlot'))
            set(handles.editPxieSlot, 'String', num2str(arbConfig.pxieSlot));
        end
        ...
        modelChange_Callback(hObject, eventdata, handles);
...
arbConfig.pxieSlot = evalin('base', get(handles.editPxieSlot, 'String'));
...
function modelChange_Callback(hObject, eventdata, handles)
arbIdx = get(handles.popupmenuModel, 'Value');
arbModelIdx = handles.arbModelPtrs{arbIdx};
arbModeIdx = get(handles.popupmenuMode, 'Value');
arbModels = handles.modelInfo(:,1);
...
if (~isempty(strfind(arbModel, 'M8195A_Rev0')))
    ...
    set(handles.editPxieSlot, 'Visible', 'off');
    set(handles.textPxieSlot, 'Visible', 'off');
elseif (~isempty(strfind(arbModel, 'M3202A')))
    set(handles.editVisaAddr, 'Enable', 'off');
    set(handles.editIPAddress, 'Enable', 'off');
    set(handles.editPort, 'Enable', 'off');
    set(handles.popupmenuConnectionType, 'Enable', 'off');
    set(handles.editPxieSlot, 'Visible', 'on');
    set(handles.textPxieSlot, 'Visible', 'on');
else
...
% M8195A Rev. 2 and M8196A
if (~isempty(strfind(arbModel, 'M8190A')) || ~isempty(strfind(arbModel, 'M8196A')) || ...
...
if (~isempty(strfind(arbModel, 'M819')) && ...
    ...
    warndlg({'For M8190A and M8195A, you have to use a VISA address that starts' ...
...
elseif (~isempty(strfind(model, 'N5194A')))
    checkmodel = 'N5194A';
elseif (~isempty(strfind(model, 'M9336A')))
...
function editPxieSlot_Callback(hObject, eventdata, handles)
% hObject    handle to editPxieSlot (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of editPxieSlot as text
%        str2double(get(hObject,'String')) returns contents of editPxieSlot as a double


% --- Executes during object creation, after setting all properties.
function editPxieSlot_CreateFcn(hObject, eventdata, handles)
% hObject    handle to editPxieSlot (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
