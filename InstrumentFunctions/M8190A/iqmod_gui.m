...
% Last Modified by GUIDE v2.5 29-Jun-2017 14:36:45
...
guidata(hObject, handles);
[iqdata sampleRate oversampling marker] = calcModIQ(handles, 'download');
if (~isempty(iqdata))
...
if (div > 1)
    ...
    [s fs oversampling marker] = calcModIQ(handles, 'download', 0, clockPat);
    if (~isempty(s))
        chMap = get(handles.pushbuttonChannelMapping, 'UserData');
...
[iqdata sampleRate oversampling marker] = calcModIQ(handles, 'none');
vsaApp = vsafunc([], 'open');
if (~isempty(vsaApp))
...
function [iqdata sampleRate oversampling marker] = calcModIQ(handles, fct, doCode, clockPat)
% handles    structure with handles and user data (see GUIDATA)
marker = [];
...
if (exist('doCode', 'var') && doCode ~= 0)
    ...
    '    ''quadErr'', %g, ''iqskew'', %g, ''gainImbalance'', %g, ''correction'', %d, ''function'', ''download'');\n' ...
    ...
    [iqdata, newSampleRate, newNumSymbols, newNumSamples] = iqmod('sampleRate', sampleRate, ...
