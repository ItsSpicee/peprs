function result = iqdownload_M8190A(arbConfig, fs, data, marker1, marker2, segmNum, keepOpen, channelMapping, sequence, run, segmentLength, segmentOffset)
...
%                if (xfprintf(f, cmd, 1) ~= 0)
        ...
        if (interpolationFactor ~= 1)
            ...
            if (sum(channelMapping(1,:)) ~= 0)
                gen_arb_IQ_M8190A(arbConfig, f, 1, data, marker1, segmNum, segmentLength, segmentOffset);
            end
            if (sum(channelMapping(2,:)) ~= 0)
                gen_arb_IQ_M8190A(arbConfig, f, 2, data, marker2, segmNum, segmentLength, segmentOffset);
            end
        else
            % direct mode waveform download
            for ch = find(channelMapping(:,1))'
                gen_arb_M8190A(arbConfig, f, ch, real(data), marker1, segmNum, run, segmentLength, segmentOffset);
            end
            for ch = find(channelMapping(:,2))'
                gen_arb_M8190A(arbConfig, f, ch, imag(data), marker2, segmNum, run, segmentLength, segmentOffset);
            end
        end
