function [ score, CalResults ] = ExtractImbalanceAndVerify_WithAWGCorrection(svars)
...
%         Received_train = Received_train(length(svars.downconversionFilterTaps)+1:end);
    ...
    Received_train = Received_train(20e3+1:end);

    % Unify the lengths of the sampled data
    [In_train, Out_train]= UnifyLength(In_train, Received_train);
    

    [d_In,d_Out, ~, phase]              = AdjustPowerAndPhase(In_train, Out_train, 0);
    [d_In,d_Out , timedelay1] = AdjustDelay_FreqDomain(d_In,d_Out, svars.fSampleCal, 5000);
%     [d_In,d_Out , timedelay2] = AdjustDelay(d_In,d_Out, svars.fSampleCal, 5000);
    [In_D,Out_D, ~, phase]              = AdjustPowerAndPhase(d_In,d_Out, 0) ;
    [In_D,Out_D, ~, phase]              = AdjustPowerAndPhase(In_D,Out_D, 0) ; 
    ...
    save('./Cal_output_MLIQ/TX_CAL_RAW_DATA_4GHz.mat', 'Received_train', 'In_train');
...
%                                 'H_offset_test', H_offset_test);
    ...
    In_test_I = real(In_test);
    In_test_Q = imag(In_test);
    % Apply AWG Correction
    tones_ref_validation = ApplyIfCal_real_imag(In_test_I, svars.fSampleTx, ...
     svars.awgCorrectionI.tonesBaseband, real(svars.awgCorrectionI.H_Tx_freq_inverse), imag(svars.awgCorrectionI.H_Tx_freq_inverse));
    In_test_I = real(tones_ref_validation);
    
    tones_ref_validation = ApplyIfCal_real_imag(In_test_Q, svars.fSampleTx, ...
     svars.awgCorrectionQ.tonesBaseband, real(svars.awgCorrectionQ.H_Tx_freq_inverse), imag(svars.awgCorrectionQ.H_Tx_freq_inverse));
    In_test_Q = real(tones_ref_validation);
    In_test_awgcal = complex(In_test_I, In_test_Q);
    tic
    [In_test_awgcal, In_test_cal] = ApplyIQImbalanceCal(In_test_awgcal, CalResults, svars.fSampleTx, svars.fSampleAWG, svars.useOffsetModel);
    toc
...
%     Received_test= digital_lpf(Received_test, svars.fSampleCal, (svars.testBW - 600e6) / 2);
%     In_test = digital_lpf(In_test, svars.fSampleCal, (svars.testBW - 600e6) / 2);
%     In_test = SetMeanPower(In_test, 0);
    ...
    Received_test = Received_test(20e3+1:end);

    [In_test, Received_test]= UnifyLength(In_test, Received_test);
    [d_In,d_Out, offset, phase]              = AdjustPowerAndPhase(In_test, Received_test, 0);
    [d_In, d_Out, timedelay1] = AdjustDelay_FreqDomain(d_In,d_Out, svars.fSampleTx, 5000);
